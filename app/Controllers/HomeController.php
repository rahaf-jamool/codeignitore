...
class HomeController extends BaseController
{
        ...
        if ($this->generalSettings->membership_plans_system == 1) {
            ...
            if ($plan->is_free == 1) {
                ...
                if ($membershipModel->addShopOpeningRequest($data)) {
                    $membershipModel->addShopOpeningEmail();
                    //go to checkout
                    helperSetSession('shop1_selected_membership_plan_id', $plan->id);
                    helperSetSession('shop1_membership_request_type', 'new');
    ...
    public function renewMembershipPlanPost()
    {
        ...
        helperSetSession('shop1_selected_membership_plan_id', $plan->id);
        helperSetSession('shop1_membership_request_type', 'renew');
        return redirect()->to(generateUrl('cart', 'payment_method') . '?payment_type=membership');
    }
    ...
    public function addConversation()
    {
        ...
        if (user()->id == $receiverId) {
            ...
            $conversationId = $messageModel->addConversation(user()->id);
            if ($conversationId) {
                $messageId = $messageModel->addMessage($conversationId, user()->id);
                if ($messageId) {
